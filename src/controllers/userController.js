...
export const postjoin = async (req, res) => {
  const { name, username, email, password, password2, location } = req.body; // form Data
...
export const finishGithubLogin = async (req, res) => {
  ...
  const config = {
    client_id: process.env.GH_CLIENT,
    client_secret: process.env.GH_SCERET,
    code: req.query.code,
  };
  const params = new URLSearchParams(config).toString();
  const finalUrl = `${baseUrl}?${params}`;
...
export const postChangepassword = async (req, res, next) => {
  ...
  if (!ok) {
    return res.status(400).render("users/change-password", {
      ...
      errorMessage: "The current password is correct",
    });
  } // check the Password between oldPassword and password

  if (newPassword !== newPasswordConfirmation) {
    return res.status(400).render("users/change-password", {
      pageTitle: "Change Password",
      errorMessage: "The password does not match the confirmation",
    }); // res.status makes browser know the error(alert current situation)
  }
  ...
  await user.save();
  req.session.user.password = user.password; // update session
  // we will make notification(send notification)
